...
namespace Ejercicio01
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            int numero,positivo = 0,negativo = 0,cero = 0,i = 1;//Variables
            while (i < 21)
            {
                
                Console.WriteLine($"({i}).Ingrese un número positivo o negativo: ");
                numero = Convert.ToInt32(Console.ReadLine());
                if (numero > 0) // número que sea mayor a cero
                {
                    positivo++;
                }  
                else if (numero < 0)  // número que sea menor a cero
                {
                    negativo++;
                }
                else
            ...
            Console.ReadKey(); //que no se cierre programa
        }
    }
}
